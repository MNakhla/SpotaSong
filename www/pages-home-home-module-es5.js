...
var GenresViewComponent = /** @class */ (function () {
    function GenresViewComponent(router) {
        this.router = router;
    }
    GenresViewComponent.prototype.ngOnInit = function () { };
    GenresViewComponent.prototype.navigate = function (genre) {
        console.log(genre);
        this.router.navigate(['songs'], { queryParams: { g: genre.label } });
    };
    GenresViewComponent.ctorParameters = function () { return [
...
var HomePage = /** @class */ (function () {
    function HomePage(router, http, spotifyService) {
        this.router = router;
        this.http = http;
    ...
    HomePage.prototype.ngOnInit = function () {
        var _this = this;
        this.spotifyService.getAllGenres().subscribe(function (response) {
            if (response) {
                _this.genres = response.genres;
                _this.genres.forEach(function (element) {
                    _this.spotifyService.getGenreImages(element).subscribe(function (res) {
                        if (res) {
                            var url = res.playlists.items[0].images[0].url;
                            // console.log(url);
                            _this.genreImages.push(url);
                            var newGenre = new Genre(element, url);
                            _this.GenreArr.push(newGenre);
                        }
                        else {
                            _this.genreImages = [];
                        }
                        console.log(_this.genreImages);
                        console.log(_this.GenreArr);
                    });
                    // ,
